...
void Drawing::DrawFilledTriangle(const Triangle &t, const Object3D &obj3d, const std::vector<std::unique_ptr<Light>> &light_vec, const Vector3 &camera_position){
    Vector4 v0 = t.a;
    Vector4 v1 = t.b;
    Vector4 v2 = t.c;
            ...
            for(int x = trunc_x_start; x <= trunc_x_end; x++){
                ...
                if(obj3d.light_type == LightingType::FLAT_LIGHTING){
                    ...
                    for(auto const &l : light_vec){
                        intensity += l->calculate_intensity(interpolated_vertex_values.vertex_position, interpolated_vertex_values.vertex_normal, camera_position, obj3d.tex_params);
                    }
                    color = (std::min((int)(intensity * color_red), 0xFF) << 24) + (std::min((int)(intensity * color_green), 0xFF) << 16) + ((int)(std::min((int)(intensity * color_blue), 0xFF)) << 8);
                    color += 0xFF;
                }
